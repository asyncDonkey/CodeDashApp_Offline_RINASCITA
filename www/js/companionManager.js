        ...
        for (const potentialTarget of currentFrameTargets) {
          ...
          if (potentialTarget && potentialTarget.health > 0) {
            const distance = Math.sqrt(
              Math.pow(potentialTarget.x - this.x, 2) + Math.pow(potentialTarget.y - this.y, 2),
            );
...
class BaseCompanion {
  ...
  constructor(
    ...
    if (this.sprite && this.sprite.complete && this.sprite.naturalWidth > 0) {
      ...
      console.warn(
        `DEBUG_COMP: Sprite mancante o rotto per il compagno ${type} (chiave cercata: ${spritePath}).`,
      );
      this.animation = null;
    }
  ...
  update(dt, collectiblesArray = []) {
    if (!this.player) {
      console.error(
        `DEBUG_COMP: BaseCompanion.update: this.player è undefined per ${this.type}! Impossibile aggiornare la posizione.`,
      );
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      if (
        this.projectiles[i].x > this.player.x + 800 ||
        this.projectiles[i].x < this.player.x - 800
      ) {
...
export class CloudAssistant extends BaseCompanion {
  ...
  update(dt, collectiblesArray = []) {
    // Accetta collectiblesArray ma non lo usa
    this.x = this.player.x + this.player.displayWidth / 2 + this.offsetX;
    this.y =
      this.player.y +
      this.initialOffsetY +
      Math.sin((Date.now() / 1000) * this.frequency) * this.amplitude;
  ...
  performAttack() {
    ...
    if (obstacles.length > 0) {
      this.projectiles.push(
        new CompanionProjectile(
          projX,
          projY,
          this.images[COMPANION_PROJECTILE_CLOUD_SRC],
          COMPANION_PROJECTILE_FRAME_WIDTH,
          COMPANION_PROJECTILE_FRAME_HEIGHT,
          COMPANION_PROJECTILE_NUM_FRAMES,
          COMPANION_PROJECTILE_TARGET_SIZE,
          COMPANION_PROJECTILE_TARGET_SIZE,
...
export class DebuggatorIA extends BaseCompanion {
  constructor(player, images) {
    ...
    this.teleportPositions = [
      ...
      {
        x: this.player.displayWidth / 2 - this.width / 2,
        y: this.player.displayHeight / 2 - this.height / 2,
      },
    ];
  ...
  update(dt, collectiblesArray = []) {
    ...
    if (!this.player || typeof this.player.displayWidth === 'undefined') {
      console.error(
        'DEBUG_COMP: DebuggatorIA.update: this.player o le sue proprietà di visualizzazione sono undefined! this.player value:',
        this.player,
      );
      console.error(new Error().stack);
      return;
...
export class Kernel42 extends BaseCompanion {
  constructor(player, images) {
    ...
    super(
      ...
      images,
      'kernel_4_2',
      COMPANION_KERNEL_4_2_SRC,
      KERNEL_4_2_FRAME_WIDTH,
      KERNEL_4_2_FRAME_HEIGHT,
      KERNEL_4_2_NUM_FRAMES,
      KERNEL_4_2_ANIMATION_SPEED,
      0.8,
    ); // Attacca più velocemente
  ...
  update(dt, collectiblesArray = []) {
    // Riceve l'array dei collezionabili qui
    this.time += dt;
    ...
    this.x =
      ...
      this.baseOffsetX +
      Math.sin(this.time * this.frequencyX) * this.amplitudeX;
    this.y =
      this.player.y + this.baseOffsetY + Math.sin(this.time * this.frequencyY) * this.amplitudeY;
  ...
  performAttack() {
    ...
    if (anyTargetExists) {
      // Spara solo se c'è potenzialmente qualcosa da colpire
      this.projectiles.push(
        new CompanionProjectile(
          projX,
          projY,
  ...
  checkAndCollect(collectiblesArray) {
    ...
    for (let i = collectiblesArray.length - 1; i >= 0; i--) {
      ...
      const collided =
        companionRect.x < collectible.x + collectible.width &&
      ...
      if (collided) {
        ...
        if (shouldCollect) {
          ...
          if (
            collectible.type === 'kiwi' ||
            collectible.type === 'orange' ||
            collectible.type === 'pear'
          ) {
            AudioManager.playSound(SFX_COMPANION_FRUIT_COLLECT);
          } else if (
            collectible.type === 'bronze' ||
            collectible.type === 'silver' ||
            collectible.type === 'gold' ||
            collectible.type === 'platinum' ||
            collectible.type === 'rich_bit' ||
            collectible.type === 'data_pkg'
          ) {
...
export class CompanionManager {
  ...
  equipCompanion(companionId, player, images) {
    console.log(
      `DEBUG_COMP: equipCompanion chiamato. companionId: ${companionId}, player object is:`,
      player,
    );
    if (this.activeCompanion) {
      console.log(
        `DEBUG_COMP: Disequipaggiamento compagno precedente ${this.activeCompanion.type}`,
      );
    ...
    if (this.activeCompanion) {
      console.log(
        `DEBUG_COMP: Compagno ${this.activeCompanion.type} impostato come compagno attivo.`,
      );
    }
  }
  ...
  unequipCompanion() {
    if (this.activeCompanion) {
      console.log(
        `DEBUG_COMP: unequipCompanion chiamato. Disequipaggiamento di ${this.activeCompanion.type}`,
      );
      this.activeCompanion.projectiles = [];
      this.activeCompanion = null;
    } else {
      console.log(
        'DEBUG_COMP: unequipCompanion chiamato, ma nessun compagno attivo da disequipaggiare.',
      );
    }
  }
