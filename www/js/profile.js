...
import {
    generateBlockieAvatar,
    updateMenuVisuals
} from './main.js'; // generateBlockieAvatar è ora in main.js
...
async function activateItemTab(modalElement, activeTabId, userData) {
    ...
    itemsToRender = unlockedItems.filter(id => {
        ...
        return item && (activeTabId === 'all' ? itemTypesInThisModal.includes(item.type) : item.type === activeTabId);
    });

    if (modalElement.id === 'skinsModal') {
        if (activeTabId === 'donkeySkin' || activeTabId === 'all') itemsToRender.push('skin_donkey_default_info');
        if (activeTabId === 'bulletSkin' || activeTabId === 'all') itemsToRender.push('skin_bullet_log');
    }
...
export const renderSkinsModal = () => setupModal('skinsModal', 'donkeySkin', 'skins-categories-nav');
export const renderCompanionsModal = () => setupModal('companionsModal', 'companion', 'companions-categories-nav');
export const renderPowerupsModal = () => setupModal('powerupsModal', 'permanentPowerup', 'powerups-categories-nav');
    ...
    try {
        ...
        if (itemType === 'permanentPowerup') {
            ...
            if (equippedPups.has(itemId)) {
                ...
                if (!currentUserData.inventory.unlockedItems.includes(itemId)) throw new Error("You don't own this power-up.");
            ...
            if (itemId === null || cosmeticCatalog[itemId]?.isDefault) {
                currentUserData.inventory.equipped[itemType] = itemType === 'donkeySkin' ? 'skin_donkey_default_info' : 'skin_bullet_log';
                showToast(`Default ${itemType} equipped.`, 'success');
            } else {
                if (!currentUserData.inventory.unlockedItems.includes(itemId)) throw new Error("You don't own this item.");
        ...
        for (const modalId in openModals) {
            ...
            if (modalElement && modalElement.style.display === 'flex') {
                ...
                const activeTabId = activeButton ? activeButton.dataset.typeFilter : (modalId === 'skinsModal' ? 'donkeySkin' : 'companion');
