...
export function initializeMenu(initialDonkeyPos = null) {
  ...
  if (menuCanvasElement && !menuAnimation.canvas) {
  ...
  if (currentUserData) {
    ...
    console.warn(
      'currentUserData not available in initializeMenu. Menu player display might not update correctly.',
    );
  }
  ...
  if (window.currentUserData && window.currentUserData.gameStats) {
    ...
    if (digitalFruitCountValueElement) {
      digitalFruitCountValueElement.textContent =
        window.currentUserData.gameStats.totalDigitalFruits || 0;
    }
  }
...
const startGameSequence = async () => {
  ...
  for (const itemType in collectedItemsToSync) {
    ...
    if (itemDefinition) {
      if (itemDefinition.value > 0 && !itemDefinition.fruitEquivalent) {
        ...
        totalFruitsForToast +=
          (itemDefinition.fruitEquivalent || 0) * (collectedItemsToSync[itemType] || 0);
      }
    } else {
  ...
  if (currentUserData && Object.keys(collectedItemsToSync).length > 0) {
    ...
    try {
      currentUserData.gameStats.totalBits =
        (currentUserData.gameStats.totalBits || 0) + totalBitsForToast;
      currentUserData.gameStats.totalDigitalFruits =
        (currentUserData.gameStats.totalDigitalFruits || 0) + totalFruitsForToast;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('menubitscollected', () => {
    ...
    for (const itemType in collectedCounts) {
      ...
      if (itemDefinition) {
        if (itemDefinition.value > 0 && !itemDefinition.fruitEquivalent) {
          ...
          pendingTotalDigitalFruits +=
            (itemDefinition.fruitEquivalent || 0) * (collectedCounts[itemType] || 0);
        }
      }
    }
    ...
    const bitCountValueElement = document.getElementById('bit-count-value');
    if (bitCountValueElement) {
      bitCountValueElement.textContent =
        (currentUserData?.gameStats?.totalBits || 0) + pendingTotalBits;
    }
    const digitalFruitCountValueElement = document.getElementById('digital-fruit-count-value');
    if (digitalFruitCountValueElement) {
      digitalFruitCountValueElement.textContent =
        (currentUserData?.gameStats?.totalDigitalFruits || 0) + pendingTotalDigitalFruits;
    }
    initializeLocalUser();
  });
  ...
  if (userAvatarIcon) {
    ...
    userAvatarIcon.addEventListener(
      'touchend',
      (e) => {
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      while (target && target !== document.body) {
        if (
          target.matches(
            'button, a, .menu-btn, .top-bar-icon-btn, [onclick], [role="button"], [tabindex]:not([tabindex="-1"])',
          )
        ) {
  ...
  setTimeout(() => {
    const isRainingInitial = window.menuAnimation
      ? window.menuAnimation.isDigitalRainActive
      : false;
    if (rainStatusIndicator) {
  ...
  async function initializeLocalUser() {
    ...
    try {
      ...
      if (!userProfile) {
        console.log(
          '[Main.js] Nessun profilo utente locale trovato. Creazione di un profilo di default.',
        );
        userProfile = {
          userId: localUserId,
      ...
      document.getElementById('bit-count-value').textContent =
        currentUserData.gameStats?.totalBits || 0;
      if (digitalFruitCounter) {
        document.getElementById('digital-fruit-count-value').textContent =
          currentUserData.gameStats?.totalDigitalFruits || 0;
      }
      ...
      console.error(
        '[Main.js] Errore critico durante il caricamento/inizializzazione utente locale:',
        error,
      );
    ...
    if (App) {
      ...
      App.addListener('resume', () => {
        ...
        if (currentGameState === GAME_STATE.MENU) {
          ...
          console.log(
            '[App Event] Gioco era in pausa. Torna allo schermo di pausa, attende Resume.',
          );
        }
      });
    } else {
      console.error(
        '[Main.js] Plugin App di Capacitor non trovato. I listener di pausa/resume non saranno attivi.',
      );
    }
  }
