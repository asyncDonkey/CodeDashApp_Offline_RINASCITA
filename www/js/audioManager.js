...
async function loadSound(name, path) {
  ...
  try {
    ...
    console.error(
      `Errore durante il caricamento o decodifica dell'audio ${name} (${path}):`,
      error,
    );
 ...
 * @returns {Promise<AudioBuffer>}
 */
async function loadBackgroundMusic(name, path) {
  // Modificato: aggiunto 'name'
  if (!audioContext) {
    console.warn('AudioContext non supportato. Audio disabilitato.');
    return Promise.reject('AudioContext not supported');
  }
...
function playMusicByName(musicName, loop = true) {
  ...
  if (
    currentBackgroundMusicName === musicName &&
    isMusicPlaying &&
