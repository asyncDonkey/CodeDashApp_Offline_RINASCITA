...
async function renderShopItems(category) {
  ...
  try {
    ...
    const itemsToDisplay = Object.entries(cosmeticCatalog)
      .map(([id, item]) => ({ id, ...item }))
      .filter((item) => item.type === category && item.price > 0 && !userUnlockedItems.has(item.id))
...
async function handlePurchase(itemId) {
  ...
  try {
    ...
    if (currencyType === 'digital_fruits') {
      if ((user.gameStats.totalDigitalFruits || 0) < item.price)
        throw new Error('Frutti digitali insufficienti.');
