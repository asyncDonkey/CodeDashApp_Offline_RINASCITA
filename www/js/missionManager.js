...
export function initMissions() {
  ...
  if (!missionUIElement) {
    console.warn(
      '[MissionManager] Elemento #mission-display non trovato. La UI delle missioni non sarà visibile.',
    );
  }
}
  ...
  switch (currentMission.type) {
    ...
    case MISSION_TYPES.DONT_TAKE_DAMAGE:
      if (playerTookDamageThisMission)
        failed = true; // Fallita se ha subito danno
...
function completeMission() {
  ...
  if (window.incrementMissionsCompleted) {
    ...
    console.warn(
      '[MissionManager] Impossibile incrementare il contatore missioni. window.incrementMissionsCompleted non definita.',
    );
  }
  ...
  console.log(
    `[MissionManager] Missione completata: ${currentMission.description}. Ricompensa: ${reward} Bit.`,
  );
...
export function updateMissionManager(dt) {
  if (!missionActive && missionCooldownTimer > 0) {
    ...
    if (
      currentMission.type === MISSION_TYPES.SURVIVE_TIME ||
      currentMission.type === MISSION_TYPES.DONT_TAKE_DAMAGE
    ) {
...
export function updateMissionDisplay() {
  if (!missionUIElement) {
    console.warn(
      '[MissionManager] missionUIElement è null. Impossibile aggiornare la UI della missione.',
    );
    return;
  }
