...
if (loginFormModal) {
  loginFormModal.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const redirectUrl =
        new URLSearchParams(window.location.search).get('redirect') || '/profile.html';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (emailInput && emailErrorDiv)
    emailInput.addEventListener('input', () => clearFieldError(emailErrorDiv));
  ...
  if (authAction === 'login') {
    if (currentLoginModal) {
      ...
      console.warn(
        "[Register.js] Azione 'login' richiesta, ma la modale di login non è stata trovata nel DOM.",
      );
    }
  } else if (authAction === 'signup') {
    console.log(
      "[Register.js] Azione 'signup' richiesta: comportamento di default (form registrazione visibile).",
    );
    if (currentLoginModal) {
    ...
    console.log(
      '[Register.js] Nessuna authAction specifica o valore non riconosciuto. Comportamento di default.',
    );
    if (currentLoginModal) {
      currentLoginModal.style.display = 'none'; // Assicurati che la modale sia chiusa
    }
